...
<?php add_action('wpo_wcpdf_before_document', 'check_Storno', 10, 2); ?>
...
<?php 
	//if( $this->has_header_logo() ) echo $this->get_title(); 
	if( $this->has_header_logo() ) {
		$order_status = apply_filters('get_order_status_hook', $this->order);
		$val = '';
					...
					<tfoot>
						...
						<tr class="<?php echo $key; ?>">
							...
							<td class="price"><span class="totals-price"><?php
								$order_status = apply_filters('get_order_status_hook', $this->order);
								if ($order_status == 'cancelled' and $key != 'tax_DE-STANDARD-1') {
									$val = '-'.strval($total['value']); 
									echo $val;
								} else {
									echo $total['value']; 
								}
...
<br>
<?php 
$payment_method = apply_filters('get_order_payment_method_hook', $this->order);
if ($payment_method == "bacs" or $payment_method == "invoice" or $payment_method == "sahu_karb_gateway") {
?>
<div class="customNotes"><p>Bitte überweisen Sie den Rechnungsbetrag mit Angabe der Rechnungs- oder Bestellnummer im Verwendungszweck innerhalb von 14 Tagen auf das unten stehende Konto.</p></div>
<?php 
} else {
?>
<div class="customNotes"><p>Vielen Dank für Ihren Einkauf.</p></div>
<?php
}
?>
